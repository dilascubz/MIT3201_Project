...
namespace DALCSCL
{
    ...
    public class DALSocial
    {
        ...
        public int SocialComments(int CommentId, string Comment, string UserName)
        {
            SqlParameter[] Parameters = new SqlParameter[3];

            Parameters[0] = new SqlParameter("@CommentID", CommentId);
            Parameters[1] = new SqlParameter("@Comment", Comment);
            Parameters[2] = new SqlParameter("@UserLoginId", UserName);

            return objDALCommon.ExecuteProcedure("SinSocialCommentsDetails", CommandType.StoredProcedure, Parameters);
        }
